...
<div id="product-<?php the_ID(); ?>" <?php wc_product_class( '', $product ); ?>>
	...
	<button id="see-more-btn" class="see-more-btn" style="display:none;margin-top:20px;">Xem thêm</button>
...
function updateSeeMoreButton() {
  ...
  if (!activeTab) { btn.style.display = 'none'; return; }
  // Xóa fade-gradient cũ nếu có
  var oldFade = activeTab.querySelector('.fade-gradient');
  if (oldFade) oldFade.remove();
  // Đo lại chiều cao
  activeTab.classList.remove('collapse-content', 'expanded');
  void activeTab.offsetHeight; // force reflow
  var contentHeight = activeTab.scrollHeight;
  if (contentHeight > maxHeight) {
    activeTab.classList.add('collapse-content');
    // Thêm gradient fade
    var fade = document.createElement('div');
    fade.className = 'fade-gradient';
    activeTab.appendChild(fade);
    btn.style.display = '';
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  btn.addEventListener('click', function() {
    var activeTab = document.querySelector('.tab-pane.active');
    if (!activeTab) return;
    var fade = activeTab.querySelector('.fade-gradient');
    var expanded = activeTab.classList.toggle('expanded');
    if (expanded) {
  ...
  // Khi chuyển tab, reset lại nút và trạng thái
  document.querySelectorAll('.nav-link[data-bs-toggle="pill"]').forEach(function(link) {
    link.addEventListener('shown.bs.tab', function(e) {
      document.querySelectorAll('.tab-pane').forEach(function(tab) {
        tab.classList.remove('expanded');
        var fade = tab.querySelector('.fade-gradient');
        if (fade) fade.style.display = '';
      });
      updateSeeMoreButton();
    });
